...
public sealed partial class DemoCatalog
{
    ...
    /// <summary>
    /// Everything playable under an item, as <c>allLeaves</c> lists it: a show's episodes season by
    /// season, a season's episodes, an artist's tracks album by album, an album's tracks; the item
    /// itself for a track.
    /// </summary>
    public IReadOnlyList<MetadataItem> AllLeaves(string ratingKey) => Find(ratingKey)?.Type switch
    {
        "show" or "artist" => [.. ChildrenOf(ratingKey).SelectMany(child => ChildrenOf(child.RatingKey))],
        "season" or "album" => ChildrenOf(ratingKey),
